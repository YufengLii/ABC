...
def get_enclosing_angle(point, knn, k):
    ...
    for j in range(k):
        for g in reversed(range(k)):
            if j != g and g <= k - 1:
                enc_angles_point.append(get_enc_angle([float(point[0]), float(point[1])], [float(knn[j][0]), float(knn[j][1])], [float(knn[g][0]), float(knn[g][1])]))
            else:
                break
...
def get_enclosing_angles(point, mean_point, knn, k):
    enc_angles_point = []
    for j in range(k):
        enc_angle_point = (get_enc_angle([float(point[0]), float(point[1])], [float(mean_point[0]), float(mean_point[1])], [float(knn[j][0]), float(knn[j][1])]))
